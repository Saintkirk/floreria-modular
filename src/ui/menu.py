...
def mostrar_documento_completo(coleccion: Collection, cliente_id, titulo: str = "") -> None:
    ...
    print(f"{Colors.CYAN}Fecha registro:{Colors.END} {fecha_reg.strftime('%d/%m/%Y') if fecha_reg else 'N/A'}")
    print(f"{Colors.CYAN}Cumpleaños:{Colors.END} {fecha_cumple.strftime('%d/%m/%Y') if fecha_cumple else 'N/A'}")
