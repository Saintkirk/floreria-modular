    ...
    while True:
        ...
        try:
            ...
            if fecha_cumple > datetime.now() or fecha_cumple.year < (datetime.now().year - MAX_EDAD_ANIOS):
                print(f"{Colors.RED}✗ Fecha fuera de rango{Colors.END}")
                continue
            break
        except ValueError:
        ...
        try:
            ...
            if 1 <= num_prod <= len(catalogo) and cant > 0:
                ...
                nuevo_pedido = {
                    ...
                    "fecha_pedido": datetime.now(),
                    "ocasion": ocasion,
                    "productos": [{"nombre": prod_sel['nombre'], "cantidad": cant, "precio": prod_sel['precio_base']}],
                    "total": total,
                    "estado": "Pendiente"
                }
...
def vista_estadisticas(coleccion: Collection) -> None:
    ...
    if finanzas:
        ...
        print(f"{Colors.YELLOW}Sin datos de pedidos{Colors.END}")